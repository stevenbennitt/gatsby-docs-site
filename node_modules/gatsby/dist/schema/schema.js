...
const mergeTypes = ({
  ...
  if (!plugin || plugin.name === `default-site-plugin`) {
    ...
    report.warn(`Plugin \`${plugin.name}\` tried to define the GraphQL type ` + `\`${typeComposer.getTypeName()}\`, which has already been defined ` + `by the plugin \`${typeComposer.getExtension(`plugin`)}\`.`);
    return false;
  }
};
